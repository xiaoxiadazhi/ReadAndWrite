...
考虑任意持续获取数据和输出结果的管道。我们希望解决这样的一般问题：何时可以安全地关闭事件时间窗口，这意味着这个窗口不需要更多数据。为此，我们想要描述管道处理无界输入数据的进展。

解决这个事件时间窗口问题的一个简单方法是简单地将我们的event-time窗口基于当前的processing time。如我们在第一章所看的，我们很快陷入麻烦——数据处理和传输不是即时的，所以处理时间和时间时间几乎不会相等。我们的管道中的任何尖峰都可能会让我们错误地把信息分配给窗口。最终，这种策略失败的原因是，我们无法对这种窗口做出任何保证。

另一种直观但是不正确的方法是考虑管道处理消息的速率。虽然这是个又去的指标，但是速率可能随输入、多种期望结果、处理的可用资源等等的变化而变化。更重要的是，速率无法回答完整性的基本问题。具体来说，当我们看到特定时间间隔内的所有消息时，速率并没有告诉我们。在现实世界的系统中，会存在消息通过系统无法取得进展的情况。这可能是瞬态错误（例如崩溃，网络故障，机器停机）或持久性错误的结果，例例如需要更改应用程序逻辑或其他手动干预以解决的应用程序级故障。当然，如果发生大量故障，处理速率指标可能是删除这个的好的代理。

我们需要更好地衡量进度。为了达到这个目的，我们对流数据做一个基础假设：*每条消息有一个相关的逻辑事件时间戳*。这个假设在持续到达无界数据的上下文中是合理的，因为这意味着会一直产生输入数据。在大多数情况下，我们可以把原始的事件发生时间当做它的逻辑事件时间戳。使用包含事件时间戳的所有输入消息，我们可以检查任何管道中的此类时间戳的分布。这样的管道可以被分发以在许多代理上并行处理并消耗输入消息而不保证各个分片之间的排序。因此，此管道中活动的正在进行的消息的事件时间戳集将形成分布，如图3-1所示

消息由管道获取、处理，最终标记完成。每条消息要么是"in-flight"，表示它被接收到但还没有完成；要么是"completed"，表示这条消息不再需要被处理了。如果我们根据事件时间来查看消息的分布，将会像图3-1所示。随着时间推移，更多的消息将被添加到右侧的"in-flight"分布中，更多来自"in-flight"分布部分的消息将变成completed，并移到"completed"分布中。

![3-1](..\picture\streaming\3-1.png)

[3-1动图]: http://www.streamingbook.net/fig/3-1

在这个分布上有个关键的点，位于"in-flight"分布的最左侧边缘，对应我们管道中未处理消息的最老的时间戳。我们用这个值去定义watermark：

* *The watermark is a monotonically increasing timestamp of the oldest work not yet completed *
* watermark是一个单调递增的时间戳，这个时间戳是尚未完成的的最老的工作的时间戳
...
完美式watermark创建使用这种方式给进入的消息分配时间戳：最终的watermark严格保证管道将不会看到来自于这个数据源的、事件时间小于watermark的数据。使用完美式watermark创建的管道不再处理迟到的数据；即，在watermark之后到达的数据都是新到达的消息。然而，完美式watermark创建需要对输入数据完全了解，这对很多现实世界的分布式数据源来说是不切实际的。这有两个能创建完美式watermark的例子：
